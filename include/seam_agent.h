 ...
 * Ring RAM: sizeof(cfl_record_t) * SEAM_RING_SIZE (default: 768 bytes).
 *
 * Quick start (3 steps):
 *   1. Implement seam_port_tick() and seam_port_write_block()
 *      Non-Cortex-M ports also implement the critical-section hooks below.
...
#if SEAM_RING_SIZE == 0 || SEAM_RING_SIZE > 255
#error "SEAM_RING_SIZE must be between 1 and 255"
#endif

/* Save and restore interrupt state so maskable ISRs cannot interleave writes.
 * NMI and nested faults must not call seam_emit(). Non-Cortex-M ports provide
 * hooks with the same save/restore semantics. */
#if defined(__ARM_ARCH_6M__) || defined(__ARM_ARCH_7M__) || \
    defined(__ARM_ARCH_7EM__) || defined(__ARM_ARCH_8M_BASE__) || \
    defined(__ARM_ARCH_8M_MAIN__)
static inline uint32_t seam_critical_enter(void)
{
    uint32_t state;
...
static inline void seam_critical_exit(uint32_t state)
{
    __asm__ volatile ("msr primask, %0" :: "r"(state) : "memory");
}
#else
extern uint32_t seam_port_enter_critical(void);
extern void seam_port_exit_critical(uint32_t state);
#define seam_critical_enter seam_port_enter_critical
#define seam_critical_exit  seam_port_exit_critical
#endif
...
uint8_t      _seam_head = 0u; /* next write slot (mod SEAM_RING_SIZE) */
uint8_t      _seam_count = 0u; /* occupied slots, saturates at ring size */
uint16_t     _seam_seq  = 0u; /* monotonic event counter              */
#else
extern cfl_record_t _seam_ring[];
extern uint8_t      _seam_head;
extern uint8_t      _seam_count;
extern uint16_t     _seam_seq;
#endif

/* ── Port functions — implement these two in your BSP ──────────────────── */

/* Return current tick without blocking (called with interrupts masked). */
...
{
    uint32_t irq_state = seam_critical_enter();
    cfl_record_t *r = &_seam_ring[_seam_head];
    ...
    _seam_head++;
    if (_seam_head == SEAM_RING_SIZE) _seam_head = 0u;
    if (_seam_count < SEAM_RING_SIZE) _seam_count++;
    seam_critical_exit(irq_state);
}
...
static inline void seam_dump_bundle(uint16_t fault_seq)
{
    ...
    uint8_t start = (n == SEAM_RING_SIZE) ? _seam_head : 0u;
